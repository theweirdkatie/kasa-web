<script lang="ts">
    ...
    onMount( async() => {
        device.state = await getDeviceState(device);
    })

    async function flip_state() {
        device.state = await flipState(device);
    }
...
<Card size="lg">
    ...
    <div transition:slide="{{duration: 600}}" class="grid grid-cols-2 mt-2 pt-2 gap-3 ml-3 border-t-2">
        <blockquote>
            <h3>MAC Address</h3>
            <p class="text-sm">{device.mac}</p>
        </blockquote>
        <blockquote>
            <h3>IP Address</h3>
            <p class="text-sm">{device.host}</p>
        </blockquote>
        <blockquote>
            <h3>Device Type</h3>
            ...
            {:else if device.deviceType === 2}
                <Strip />
            {/if}
            <p class="text-sm">{device.name}</p></div>
        </blockquote>
        <blockquote class="col-span-2">
            <h3>Device ID</h3>
            <p class="text-sm">{device.deviceId}</p>
        </blockquote>
    </div>
    {/if}
</Card>